...
public class PlayerMove : MonoBehaviour
{
    ...
    {
                                    // 우리는 스크립트를 작성할 때, Input.GetKey(KeyCode.키) 라는 함수를 통해 해당 키가 눌려있는지 확인할 수 있어요.
        if (Input.GetKey(KeyCode.W))// W 키를 누르면 이 부분을 실행해요.
        {      
            MoveObjectToUp();       // 이 부분은 오브젝트를 위로 움직여요.
        }
        if (Input.GetKey(KeyCode.S))// S 키를 누르면 이 부분을 실행해요.
        {                
            MoveObjectToDown();     // 이 부분은 오브젝트를 아래로 움직여요.
        }
        if (Input.GetKey(KeyCode.A))// A 키를 누르면 이 부분을 실행해요.
        {                   
            MoveObjectToLeft();     // 이 부분은 오브젝트를 왼쪽으로 움직여요.
        }
        if (Input.GetKey(KeyCode.D))// D 키를 누르면 이 부분을 실행해요.
        {                    
